...
const FieldPackDialog=lazy(()=>import('./components/FieldPackDialog'))
const TableToolbar=lazy(()=>import('./components/TableToolbar'))
const FieldEditor=lazy(()=>import('./components/FieldEditor'))

export default function App(){
  const [exportOpen,setExportOpen]=useState(false);const[poolOpen,setPoolOpen]=useState(false);const[projectOpen,setProjectOpen]=useState(false);const[schemaOpen,setSchemaOpen]=useState(false);const[commandOpen,setCommandOpen]=useState(false);const[fieldPacksOpen,setFieldPacksOpen]=useState(false);const {project,chooseTemplate,updateProject,generate,setPanel,undo,redo,past,future,result,isGenerating}=useAppStore()
  ...
  const commands=useMemo<PaletteCommand[]>(()=>{
    ...
    return[
      ...
      {id:'field-packs',label:'添加字段套餐',hint:'身份、地址、审计、追踪、金额等常用字段组',group:'快捷操作',keywords:'field pack batch 套餐 快速建表',action:()=>setFieldPacksOpen(true)},
      ...
      ...TEMPLATES.map(template=>({id:`template-${template.templateId}`,label:`切换到${template.name}`,hint:`${template.tables.length} 表 · ${plannedProjectTotal(template)} 条默认数据`,group:'业务场景',keywords:`${template.templateId} ${template.description}`,action:()=>chooseTemplate(template.templateId)})),
  ...
  useEffect(()=>{const handler=(event:KeyboardEvent)=>{const modifier=event.metaKey||event.ctrlKey,target=event.target as HTMLElement|null,editing=target?.matches('input, textarea, select, [contenteditable="true"]'),dialogOpen=exportOpen||projectOpen||schemaOpen||poolOpen||fieldPacksOpen||commandOpen;if(modifier&&event.key.toLocaleLowerCase()==='k'){event.preventDefault();setCommandOpen(open=>!open)}else if(dialogOpen)return;else if(modifier&&event.key==='Enter'&&!editing){event.preventDefault();run()}else if(modifier&&event.shiftKey&&event.key.toLocaleLowerCase()==='e'&&!editing){event.preventDefault();setExportOpen(true)}else if(modifier&&event.shiftKey&&event.key.toLocaleLowerCase()==='p'&&!editing){event.preventDefault();setProjectOpen(true)}else if(modifier&&event.key.toLocaleLowerCase()==='z'&&!editing){event.preventDefault();event.shiftKey?redo():undo()}};window.addEventListener('keydown',handler);return()=>window.removeEventListener('keydown',handler)},[commandOpen,exportOpen,fieldPacksOpen,isGenerating,poolOpen,project.mode,projectOpen,schemaOpen,generate,setPanel,undo,redo])
  return <div className="app-shell"><Header onExport={()=>setExportOpen(true)} onProject={()=>setProjectOpen(true)} onSchemaImport={()=>setSchemaOpen(true)} onCommand={()=>setCommandOpen(true)}/><div className="workspace"><Sidebar onPool={()=>setPoolOpen(true)} onFieldPacks={()=>setFieldPacksOpen(true)}/><div className="center-column"><Suspense fallback={<div className="table-toolbar toolbar-loading"/>}><TableToolbar/></Suspense><Suspense fallback={<main className="editor empty-state"><p>加载字段设计器…</p></main>}><FieldEditor/></Suspense></div><Preview/></div><ExportDialog open={exportOpen} onClose={()=>setExportOpen(false)}/><PoolDialog open={poolOpen} onClose={()=>setPoolOpen(false)}/><ProjectDialog open={projectOpen} onClose={()=>setProjectOpen(false)}/><SchemaImportDialog open={schemaOpen} onClose={()=>setSchemaOpen(false)}/>{commandOpen&&<Suspense fallback={null}><CommandPalette open onClose={()=>setCommandOpen(false)} commands={commands}/></Suspense>}{fieldPacksOpen&&<Suspense fallback={null}><FieldPackDialog open onClose={()=>setFieldPacksOpen(false)}/></Suspense>}<PwaStatus/></div>
}
