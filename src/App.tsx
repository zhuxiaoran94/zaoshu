...
export default function App(){
  ...
  return <div className="app-shell"><Header onExport={()=>setExportOpen(true)} onProject={()=>setProjectOpen(true)} onSchemaImport={()=>setSchemaOpen(true)} onCommand={()=>setCommandOpen(true)}/><div className="workspace"><Sidebar onPool={()=>setPoolOpen(true)} onFieldPacks={()=>setFieldPacksOpen(true)}/><div className="center-column"><Suspense fallback={<div className="table-toolbar toolbar-loading"/>}><TableToolbar/></Suspense><FieldEditor/></div><Preview/></div><ExportDialog open={exportOpen} onClose={()=>setExportOpen(false)}/><PoolDialog open={poolOpen} onClose={()=>setPoolOpen(false)}/><ProjectDialog open={projectOpen} onClose={()=>setProjectOpen(false)}/><SchemaImportDialog open={schemaOpen} onClose={()=>setSchemaOpen(false)}/>{commandOpen&&<Suspense fallback={null}><CommandPalette open onClose={()=>setCommandOpen(false)} commands={commands}/></Suspense>}{fieldPacksOpen&&<Suspense fallback={null}><FieldPackDialog open onClose={()=>setFieldPacksOpen(false)}/></Suspense>}<PwaStatus/></div>
}
