...
async function loadProject(options:CliOptions):Promise<ProjectSchema>{let project:ProjectSchema;if(options.config){const path=resolve(options.config),info=await stat(path);if(!info.isFile())throw new Error(`配置路径不是普通文件：${path}`);if(info.size>MAX_CONFIG_BYTES)throw new Error('配置文件不能超过 1 MB');project=parseProjectFile(await readFile(path,'utf8'))}else{if(!TEMPLATES.some(item=>item.templateId===options.template))throw new Error(`未知模板：${options.template}；可用 --list-templates 查看`);project=cloneTemplate(options.template)}if(options.seed!==undefined)project.seed=options.seed;if(options.mode)project.mode=options.mode;const count=options.count;if(count!==undefined)project.tables.forEach(table=>table.count=count);return parseProjectFile(serializeProject(project))}
