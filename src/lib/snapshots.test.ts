...
describe('项目快照与差异',()=>{
  ...
  it('识别项目、表和字段级变化',()=>{
    ...
    after.seed=99;after.referenceDate='2030-01-01T00:00:00.000Z';after.tables[0].count+=10;after.tables[0].assertions=[{id:'id-required',name:'ID 必填',expression:'id != null',message:'ID 不能为空',severity:'error'}];after.tables[0].fields[0].generator='uuid';after.tables.splice(1,1);after.tables.push({id:'new_table',name:'events',label:'事件',count:3,fields:[{id:'new_id',name:'id',label:'ID',generator:'autoId',dataType:'number'}]})
    const changes=diffProjects(before,after)
    expect(changes.some(change=>change.id==='project-seed')).toBe(true)
    expect(changes.some(change=>change.id==='project-reference-date')).toBe(true)
    expect(changes.some(change=>change.id===`table-change-${before.tables[0].id}`&&change.detail.includes('业务断言'))).toBe(true)
