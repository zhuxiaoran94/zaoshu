...
describe('Mock API 交付包',()=>{
  ...
  it('交付可独立运行的工程文件，数据去除内部异常元信息，契约包含 CRUD 和网络配置',async()=>{
    ...
    expect(JSON.parse(openapi)['x-mock-api-behavior']).toEqual({...options,validateForeignKeys:true,deletePolicy:'restrict',nestedRoutes:true,routeOverrides:[]})
    ...
    expect(pack.manifest.mockApi).toEqual({...options,validateForeignKeys:true,deletePolicy:'restrict',nestedRoutes:true,routeOverrides:[]})
