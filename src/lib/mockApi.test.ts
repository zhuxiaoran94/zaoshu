...
const executeHandlers=(source:string,options:ReturnType<typeof normalizeMockApiOptions>)=>{
  const http=Object.fromEntries(['get','post','patch','delete'].map(method=>[method,(path:string,resolver:MockHandler['resolver'])=>({method,path,resolver})])),HttpResponse={json:(body:unknown,init?:ResponseInit)=>Response.json(body,init)},module={exports:{} as {handlers:MockHandler[];db:Record<string,Record<string,unknown>[]>;requestLog:Array<Record<string,unknown>>;resetMockData:()=>void}},compiled=transpileModule(source,{compilerOptions:{target:ScriptTarget.ES2022,module:ModuleKind.CommonJS}}).outputText
