...
export function analyzeCoverage(project:ProjectSchema,data:GeneratedData):CoverageAnalysis {
  ...
  for(const table of project.tables){
    ...
    for(const field of table.fields){
      const present=rows.filter(row=>Object.prototype.hasOwnProperty.call(row,field.name)),values=present.map(row=>row[field.name]),seen=new Set(values.map(key))
      ...
      if(candidates?.length&&!field.formula&&!field.condition&&!field.ref&&!field.prefix&&!field.suffix&&!field.unique&&!field.primaryKey){
        const missing=candidates.filter(value=>!seen.has(key(value)));enumTotal+=candidates.length;enumCovered+=candidates.length-missing.length
        if(missing.length)gaps.push({id:`enum-${field.id}`,kind:'enum',tableId:table.id,fieldId:field.id,label:`${table.label}.${field.label} 枚举值`,detail:`缺少 ${missing.length}/${candidates.length} 个候选值`,missingValues:missing})
      }
      ...
      if(field.dataType==='number'&&!field.formula&&!field.condition&&!field.ref&&!field.unique&&!field.primaryKey){
        ...
        if(expected.length){const missing=expected.filter(value=>!seen.has(key(value)));boundaryTotal+=expected.length;boundaryCovered+=expected.length-missing.length;if(missing.length)gaps.push({id:`boundary-${field.id}`,kind:'boundary',tableId:table.id,fieldId:field.id,label:`${table.label}.${field.label} 数值边界`,detail:`缺少边界值 ${missing.join('、')}`,missingValues:missing})}
      }
    }
  }
