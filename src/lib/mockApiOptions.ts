...
export const DEFAULT_MOCK_API_OPTIONS:MockApiOptions={latencyMinMs:0,latencyMaxMs:0,failureRate:0,failureStatus:503,envelope:'plain'}
...
export function normalizeMockApiOptions(value?:Partial<MockApiOptions>):MockApiOptions{
  ...
  return{latencyMinMs,latencyMaxMs,failureRate:bounded(value?.failureRate,0,100,DEFAULT_MOCK_API_OPTIONS.failureRate),failureStatus:bounded(value?.failureStatus,400,599,DEFAULT_MOCK_API_OPTIONS.failureStatus),envelope:['plain','data','data-meta'].includes(value?.envelope??'')?value!.envelope!:DEFAULT_MOCK_API_OPTIONS.envelope}
}
