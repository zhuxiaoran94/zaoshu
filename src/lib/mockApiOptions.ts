...
export type MockApiMethod='GET'|'POST'|'PATCH'|'DELETE'
...
export const DEFAULT_MOCK_API_OPTIONS:MockApiOptions={latencyMinMs:0,latencyMaxMs:0,failureRate:0,failureStatus:503,envelope:'plain',validateForeignKeys:true,deletePolicy:'restrict',nestedRoutes:true,routeOverrides:[]}
...
const normalizeRouteOverrides=(value:unknown)=>{const normalized=new Map<string,MockApiRouteOverride>();if(!Array.isArray(value))return[];for(const candidate of value.slice(0,MAX_MOCK_ROUTE_OVERRIDES)){if(!candidate||typeof candidate!=='object')continue;const item=candidate as Partial<MockApiRouteOverride>,method=String(item.method??'').toUpperCase() as MockApiMethod,path=normalizePath(item.path);if(!['GET','POST','PATCH','DELETE'].includes(method)||!path)continue;const latencyMinMs=bounded(item.latencyMinMs,0,10_000,0),latencyMaxMs=Math.max(latencyMinMs,bounded(item.latencyMaxMs,0,10_000,latencyMinMs));normalized.set(`${method} ${path}`,{method,path,latencyMinMs,latencyMaxMs,failureRate:bounded(item.failureRate,0,100,0),failureStatus:bounded(item.failureStatus,400,599,503),failureMessage:normalizeMessage(item.failureMessage)})}return[...normalized.values()]}
...
export function mockApiRouteKeys(project:ProjectSchema,nestedRoutes=true){const keys=new Set<string>();for(const table of project.tables){const base=`/api/${table.name}`;for(const method of ['GET','POST'] as const)keys.add(`${method} ${base}`);for(const method of ['GET','PATCH','DELETE'] as const)keys.add(`${method} ${base}/:id`);keys.add(`POST ${base}/_batch`)}if(nestedRoutes)for(const child of project.tables)for(const field of child.fields.filter(candidate=>candidate.ref)){const parent=project.tables.find(table=>table.id===field.ref!.tableId);if(!parent)continue;const duplicate=child.fields.filter(candidate=>candidate.ref?.tableId===parent.id).length>1;keys.add(`GET /api/${parent.name}/:id/${child.name}${duplicate?`/by-${field.name}`:''}`)}return keys}
