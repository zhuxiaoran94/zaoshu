...
const csvCell=(v:unknown,delimiter=',')=>{const s=v==null?'':typeof v==='object'?JSON.stringify(v):String(v);return s.includes(delimiter)||/["\n]/.test(s)?`"${s.replaceAll('"','""')}"`:s}
...
export function toSQL(project:ProjectSchema,data:GeneratedData,dialect:'mysql'|'postgres'|'sqlite') {
  const quote=dialect==='mysql'?'`':'"'; const lines=[dialect==='mysql'?'SET FOREIGN_KEY_CHECKS=0;':'BEGIN;']
  for(const table of project.tables){const rows=rowsFor(data,table.id).map(clean);if(!rows.length)continue;const keys=Object.keys(rows[0]);lines.push(`DELETE FROM ${quote}${table.name}${quote};`);for(let i=0;i<rows.length;i+=100){const batch=rows.slice(i,i+100);lines.push(`INSERT INTO ${quote}${table.name}${quote} (${keys.map(k=>`${quote}${k}${quote}`).join(', ')}) VALUES\n${batch.map(r=>`(${keys.map(k=>sqlValue(r[k],dialect)).join(', ')})`).join(',\n')};`)}}
...
export async function exportData(format:string,project:ProjectSchema,data:GeneratedData,report:GenerationReport,activeTableId:string) {
  ...
  else if(format==='xlsx'){const XLSX=await import('xlsx');const wb=XLSX.utils.book_new();project.tables.forEach(t=>XLSX.utils.book_append_sheet(wb,XLSX.utils.json_to_sheet(rowsFor(data,t.id).map(clean)),t.name.slice(0,31)));XLSX.writeFile(wb,`${base}.xlsx`)}
