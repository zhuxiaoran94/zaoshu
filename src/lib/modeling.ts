...
export function validate(project:ProjectSchema,data:GeneratedData):QualityCheck[] {
  ...
  for(const table of project.tables) {
    ...
    for(const field of table.fields) {
      ...
      if(field.ref) { const parentRows=data[field.ref.tableId]||[],parents=new Set(parentRows.map(r=>r[field.ref!.field])),invalidRows=rows.map((row,index)=>({value:row[field.name],index})).filter(item=>item.value!==undefined&&item.value!==null&&!parents.has(item.value)).map(item=>item.index); checks.push({id:`fk-${field.id}`,label:`${table.label}.${field.label} 引用完整性`,status:invalidRows.length===0?'pass':project.mode==='exception'?'expected':'fail',detail:invalidRows.length?`${invalidRows.length} 个无效引用`:'所有引用均有效',tableId:table.id,fieldId:field.id,...issueRows(invalidRows)});const distribution=analyzeReferenceDistribution(rows,field.name,parentRows,field.ref.field),strategy=referenceStrategy(field.ref);if(strategy==='oneToOne')checks.push({id:`fk-one-${field.id}`,label:`${table.label}.${field.label} 一对一唯一性`,status:expectedStatus(project,distribution.duplicateRows.length),detail:distribution.duplicateRows.length?`${distribution.duplicateRows.length} 条记录重复引用同一父记录`:`${distribution.assigned} 条引用互不重复`,tableId:table.id,fieldId:field.id,...issueRows(distribution.duplicateRows)});if(strategy==='roundRobin'){const balanced=distribution.maximum-distribution.minimum<=1,canGuarantee=!(field.nullable||field.missing),invalid=balanced?0:1;checks.push({id:`fk-balance-${field.id}`,label:`${table.label}.${field.label} 轮询均衡性`,status:balanced?'pass':canGuarantee?expectedStatus(project,invalid):'warning',detail:balanced?`每个父记录被引用 ${distribution.minimum}–${distribution.maximum} 次`:`引用次数相差 ${distribution.maximum-distribution.minimum}${canGuarantee?'，不符合轮询均匀策略':'；随机空值或缺失会影响最终均衡度'}`,tableId:table.id,fieldId:field.id})} }
