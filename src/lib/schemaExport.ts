...
export function toOpenAPI(project:ProjectSchema,options?:Partial<MockApiOptions>){
  const behavior=normalizeMockApiOptions(options),latencyHeader={'X-Mock-Latency':{description:'Mock 实际等待毫秒数',schema:{type:'integer'}}},errorSchema={type:'object',properties:{error:{type:'object',properties:{status:{type:'integer'},message:{type:'string'}},required:['status','message']}},required:['error']}
  ...
  const paths=Object.fromEntries(project.tables.flatMap(table=>{
    const primary=table.fields.find(field=>field.primaryKey)??table.fields.find(field=>field.unique)??table.fields[0],schemaRef={$ref:`#/components/schemas/${table.name}`},listSchema={type:'array',items:schemaRef},listPath=`/api/${table.name}`,detailPath=`${listPath}/{id}`,tag=table.label,pathParameters=[{name:'id',in:'path',required:true,schema:{type:primary.dataType==='number'?'number':'string'}}],bodySchema={type:'object',additionalProperties:false,properties:tableJsonSchema(table).properties},injected=behavior.failureRate>0?failure(behavior.failureStatus,`按 ${behavior.failureRate}% 确定性注入的网络失败`):{}
    ...
    const detail={get:{tags:[tag],summary:`查询单条${table.label}`,parameters:pathParameters,responses:{...success(schemaRef,'查询成功'),...failure(404,'记录不存在'),...injected}},patch:{tags:[tag],summary:`修改${table.label}`,parameters:pathParameters,requestBody:{required:true,content:{'application/json':{schema:bodySchema}}},responses:{...success(schemaRef,'修改成功'),...failure(400,'请求体不是 JSON 对象'),...failure(404,'记录不存在'),...injected}},delete:{tags:[tag],summary:`删除${table.label}`,parameters:pathParameters,responses:{...success(schemaRef,'删除成功'),...failure(404,'记录不存在'),...injected}}}
    return[[listPath,list],[detailPath,detail]]
  }))
