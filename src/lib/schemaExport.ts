...
export function toOpenAPI(project:ProjectSchema){const paths=Object.fromEntries(project.tables.flatMap(table=>{const primary=table.fields.find(field=>field.primaryKey)??table.fields.find(field=>field.unique)??table.fields[0],schemaRef=`#/components/schemas/${table.name}`,listPath=`/api/${table.name}`,detailPath=`${listPath}/{id}`,tag=table.label,list={get:{tags:[tag],summary:`查询${table.label}列表`,parameters:[['_page','integer'],['_limit','integer'],['_sort','string'],['_order','string'],['q','string']].map(([name,type])=>({name,in:'query',required:false,schema:{type}})),responses:{'200':{description:'分页列表',headers:{'X-Total-Count':{schema:{type:'integer'}}},content:{'application/json':{schema:{type:'array',items:{$ref:schemaRef}}}}}}},post:{tags:[tag],summary:`新增${table.label}`,requestBody:{required:true,content:{'application/json':{schema:{$ref:schemaRef}}}},responses:{'201':{description:'创建成功',content:{'application/json':{schema:{$ref:schemaRef}}}},'409':{description:'主键已存在'}}}},detail={get:{tags:[tag],summary:`查询单条${table.label}`,parameters:[{name:'id',in:'path',required:true,schema:{type:primary.dataType==='number'?'number':'string'}}],responses:{'200':{description:'查询成功',content:{'application/json':{schema:{$ref:schemaRef}}}},'404':{description:'记录不存在'}}},patch:{tags:[tag],summary:`修改${table.label}`,parameters:[{name:'id',in:'path',required:true,schema:{type:primary.dataType==='number'?'number':'string'}}],requestBody:{required:true,content:{'application/json':{schema:{type:'object',additionalProperties:true}}}},responses:{'200':{description:'修改成功',content:{'application/json':{schema:{$ref:schemaRef}}}},'404':{description:'记录不存在'}}},delete:{tags:[tag],summary:`删除${table.label}`,parameters:[{name:'id',in:'path',required:true,schema:{type:primary.dataType==='number'?'number':'string'}}],responses:{'200':{description:'删除成功',content:{'application/json':{schema:{$ref:schemaRef}}}},'404':{description:'记录不存在'}}}};return[[listPath,list],[detailPath,detail]]}));return{openapi:'3.1.0',info:{title:`${project.name} Mock API`,description:project.description,version:project.version},servers:[{url:'http://localhost',description:'MSW 本地拦截'}],paths,components:{schemas:Object.fromEntries(project.tables.map(table=>[table.name,tableJsonSchema(table)]))},'x-mock-project':{templateId:project.templateId,seed:project.seed,referenceDate:project.referenceDate,mode:project.mode}}}
