...
export function fieldJsonSchema(field:FieldRule){
  ...
  if(field.ref)schema['x-foreign-key']={tableId:field.ref.tableId,field:field.ref.field}
...
export function relationMarkdown(project:ProjectSchema){const relations=project.tables.flatMap(table=>table.fields.filter(field=>field.ref).map(field=>{const target=project.tables.find(candidate=>candidate.id===field.ref!.tableId);return`- \`${table.name}.${field.name}\` → \`${target?.name??field.ref!.tableId}.${field.ref!.field}\``}));return`# ${project.name} Schema 关系说明\n\n- Schema 版本：${project.version}\n- 随机种子：${project.seed}\n- 造数模式：${project.mode}\n- 数据表：${project.tables.length}\n- 字段：${project.tables.reduce((sum,table)=>sum+table.fields.length,0)}\n\n## 数据表\n\n${sortTables(project.tables).map((table,index)=>`${index+1}. **${table.label}**（\`${table.name}\`）：${table.fields.length} 字段，默认 ${table.count} 条`).join('\n')}\n\n## 外键关系\n\n${relations.length?relations.join('\n'):'- 当前项目没有外键关系。'}\n`}
