...
export function toOpenAPI(project:ProjectSchema,options?:Partial<MockApiOptions>){
  const behavior=normalizeMockApiOptionsForProject(project,options),traceHeaders={'X-Mock-Request-Id':{description:'稳定请求编号，用于本地请求轨迹定位',schema:{type:'string'}},'X-Mock-Latency':{description:'Mock 实际等待毫秒数',schema:{type:'integer'}}},errorSchema={type:'object',properties:{error:{type:'object',properties:{status:{type:'integer'},message:{type:'string'},index:{type:'integer',minimum:0}},required:['status','message']}},required:['error']}
  ...
  const wrapped=(schema:Record<string,unknown>,list=false):Record<string,unknown>=>wrappedWithMeta(schema,list?{type:'object',properties:{page:{type:'integer'},limit:{type:'integer'},total:{type:'integer'}},required:['page','limit','total']}:{type:'object'})
  const success=(schema:Record<string,unknown>,description:string,status='200',list=false)=>({[status]:{description,headers:{...traceHeaders,...(list?{'X-Total-Count':{schema:{type:'integer'}}}:{})},content:{'application/json':{schema:wrapped(schema,list)}}}})
  ...
  const summarySchema={type:'object',properties:{status:{type:'string',enum:['ok']},seed:{type:'integer'},tables:{type:'integer'},requests:{type:'integer'},rows:{type:'object',additionalProperties:{type:'integer'}}},required:['status','seed','tables','requests','rows']},controlResponse={description:'Mock 数据状态',content:{'application/json':{schema:wrapped(summarySchema)}}},requestLogSchema={type:'object',properties:{id:{type:'string'},sequence:{type:'integer'},method:{type:'string'},path:{type:'string'},status:{type:'integer'},latencyMs:{type:'integer'},injectedFailure:{type:'boolean'},routeOverride:{type:'string'}},required:['id','sequence','method','path','status','latencyMs','injectedFailure']}
