...
describe('Mock造数 CLI',()=>{
  ...
  it('解析 Mock API 网络和关系场景并拒绝格式误用',()=>{expect(parseCliArgs(['--format','mock-api','--mock-api-latency','120:480','--mock-api-failure-rate','15','--mock-api-failure-status','429','--mock-api-envelope','data-meta','--mock-api-delete','cascade','--mock-api-no-fk-check','--mock-api-no-nested']).mockApi).toEqual({latencyMinMs:120,latencyMaxMs:480,failureRate:15,failureStatus:429,envelope:'data-meta',validateForeignKeys:false,deletePolicy:'cascade',nestedRoutes:false});expect(()=>parseCliArgs(['--format','csv','--mock-api-failure-rate','10'])).toThrow(/只能与/);expect(()=>parseCliArgs(['--format','mock-api','--mock-api-latency','500:100'])).toThrow(/不能小于/);expect(()=>parseCliArgs(['--format','mock-api','--mock-api-envelope','wat'])).toThrow(/仅支持/);expect(()=>parseCliArgs(['--format','mock-api','--mock-api-delete','wat'])).toThrow(/仅支持/)})
