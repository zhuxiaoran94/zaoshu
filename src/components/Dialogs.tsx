...
export function ProjectDialog({open,onClose}:{open:boolean;onClose:()=>void}) {
  const {project,pools,setProject,setPools,past,future,undo,redo}=useAppStore();const input=useRef<HTMLInputElement>(null);const [error,setError]=useState('');const [message,setMessage]=useState('');const[snapshots,setSnapshots]=useState<ProjectSnapshot[]>([]);const[snapshotName,setSnapshotName]=useState('');const[selectedSnapshotId,setSelectedSnapshotId]=useState('');useEffect(()=>{if(open)snapshotStore.load().then(setSnapshots).catch(()=>setError('无法读取浏览器中的项目快照'))},[open]);if(!open)return null
  ...
  const clear=async()=>{if(!window.confirm('确定清除当前项目、自定义数据池、项目快照和浏览器本地配置吗？此操作无法撤销。'))return;localStorage.removeItem('mock-tool-ui');await snapshotStore.clear();setSnapshots([]);setPools([]);setProject(cloneTemplate('commerce'));setMessage('本地数据已清除，已恢复默认电商模板。');setError('')}
