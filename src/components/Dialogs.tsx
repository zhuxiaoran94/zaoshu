...
export function ExportDialog({open,onClose}:{open:boolean;onClose:()=>void}) {
  ...
  const formats=[['schema','Schema 契约包','无需生成 · JSON Schema / OpenAPI / TS / DDL'],['mock-api','Mock API 项目','MSW v2 · CRUD / 批量回滚 / 关系 / OpenAPI'],['bundle','完整交付包','JSON / CSV / SQL / 报告'],['json','JSON','完整多表结构'],['jsonl','JSON Lines','当前表逐行 JSON'],['csv','CSV','当前表，通用表格'],['tsv','TSV','当前表，制表符分隔'],['yaml','YAML','完整多表结构'],['xml','XML','完整多表结构'],['xlsx','Excel XLSX','多表工作簿'],['mysql','MySQL SQL','批量 INSERT 脚本'],['postgres','PostgreSQL SQL','批量 INSERT 脚本'],['sqlite','SQLite SQL','批量 INSERT 脚本'],['postman','Postman 数据','Collection Runner'],['playwright','Playwright','TypeScript Fixture'],['cypress','Cypress','TypeScript Fixture'],['jest','Jest','TypeScript Fixture'],['pytest','pytest','参数化测试文件'],['junit','JUnit','参数化 CSV'],['markdown','覆盖报告','Markdown 文档']]
  const run=async()=>{if(format==='schema'){const{exportSchemaContract}=await import('../lib/schemaExport');await exportSchemaContract(s.project)}else{if(!s.result)return;await exportData(format,s.project,s.result.data,s.result.report,s.activeTableId,{mockApi})}onClose()}
  const canExport=format==='schema'||Boolean(s.result)
  return <div className="modal-backdrop" onMouseDown={e=>e.target===e.currentTarget&&onClose()}><section className="modal export-modal"><header><div><span>EXPORT CENTER</span><h2>导出数据与契约</h2><p>所有格式均打包为 ZIP，并附 Manifest、SHA-256 和生成元信息。</p></div><button className="icon-button" onClick={onClose}><X/></button></header><div className="format-grid">{formats.map(([id,name,hint])=><button key={id} className={`${format===id?'active':''} ${id!=='schema'&&!s.result?'requires-data':''}`} onClick={()=>setFormat(id)}><FileJson/><span><strong>{name}</strong><small>{hint}</small></span></button>)}</div>{format==='mock-api'&&<Suspense fallback={<div className="mock-api-options">正在载入网络场景…</div>}><MockApiOptionsForm value={mockApi} onChange={setMockApi}/></Suspense>}<div className="export-summary"><span>{s.project.name}</span>{s.result?<code>{s.result.report.totalRows.toLocaleString()} rows</code>:<code>尚未生成数据</code>}<code>seed {s.project.seed}</code><code>date {(s.project.referenceDate??'2026-08-14').slice(0,10)}</code><code>schema {s.project.version}</code></div>{format!=='schema'&&!s.result&&<div className="dialog-message error">该格式需要先生成数据；Schema 契约包可直接导出当前设计。</div>}<footer><button className="button ghost" onClick={onClose}>取消</button><button className="button primary" disabled={!canExport} onClick={run}><Download/>{format==='schema'?'下载契约 ZIP':'下载 ZIP'}</button></footer></section></div>
}
